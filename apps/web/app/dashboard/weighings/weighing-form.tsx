...
export function WeighingForm({
  ...
  return (
    <form action={createWeighing} className="mt-6 max-w-lg space-y-4 rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-6">
      {error && (
      ...
      {transportPerKg > 0 && (
        <label className="flex items-start gap-2 rounded-md bg-stone-50 dark:bg-stone-950 px-3 py-2 text-sm font-medium">
          ...
          <span>
            ...
            {transportDeduction && <span className="ml-1 text-stone-800 dark:text-stone-200">= LKR {transportDeduction}</span>}
      ...
      {isCollector ? (
        <p className="rounded-md bg-stone-50 dark:bg-stone-950 px-3 py-2 text-sm text-stone-600 dark:text-stone-400">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <a href="/dashboard/weighings" className="rounded-md border border-stone-300 dark:border-stone-600 px-4 py-2 text-sm hover:bg-stone-100 dark:hover:bg-stone-800">
          Cancel
        </a>
      </div>
    </form>
