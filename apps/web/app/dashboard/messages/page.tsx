...
export default async function MessagesPage({
  ...
  return (
    <div className="space-y-8">
      ...
      {sp.notice && (
        <div className="rounded-md border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">{sp.notice}</div>
      )}

      <form action={sendMessage} className="space-y-4 rounded-xl border border-stone-200 bg-white p-5">
        <div>
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-stone-500">Recent</h2>
        <div className="mt-3 divide-y divide-stone-100 rounded-xl border border-stone-200 bg-white">
          {sent.map((m) => (
            <div key={m.id} className="px-4 py-3">
              ...
              <p className="mt-1 text-sm text-stone-600">{m.body}</p>
              <p className="mt-1 text-xs text-stone-400">{new Date(m.sent_at).toLocaleString()}</p>
            </div>
          ))}
          {sent.length === 0 && <p className="px-4 py-8 text-center text-sm text-stone-400">No messages sent yet.</p>}
        </div>
      </section>
    </div>
  );
