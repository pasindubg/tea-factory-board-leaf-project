...
export async function createUser(formData: FormData) {
  ...
  if (authErr || !created.user) {
    const msg = authErr?.code === "email_exists"
      ? "An account with this email already exists."
      : friendlyError(authErr);
  ...
  if (insErr) {
    ...
    redirect(`/dashboard/users/new?error=${encodeURIComponent(friendlyError(insErr))}`);
  }
    ...
    if (colErr) {
      redirect(
        `/dashboard/users?notice=${encodeURIComponent(
          `User created, but their collector record failed (${friendlyError(colErr)}). Create and link one on the Collectors page.`,
        )}`,
      );
    }
  }
...
export async function setUserActive(formData: FormData) {
  ...
  const { data: updated, error } = await supabase
    ...
    .single();
  if (error || !updated) {
    redirect(`/dashboard/users?error=${encodeURIComponent(friendlyError(error ?? 'User not found.'))}`);
  }
  ...
  if (banErr) {
    redirect(
      `/dashboard/users?notice=${encodeURIComponent(
        `${updated.name} ${nextActive ? "reactivated" : "deactivated"} in the app, but the login ban failed: ${friendlyError(banErr)}`,
      )}`,
    );
  }
...
export async function removeUser(formData: FormData) {
  ...
  if (authErr && authErr.code !== "user_not_found") {
    redirect(`/dashboard/users?error=${encodeURIComponent(`Could not delete the login: ${friendlyError(authErr)}`)}`);
  }
  ...
  if (delErr) {
    redirect(`/dashboard/users?error=${encodeURIComponent(friendlyError(delErr))}`);
  }
...
export async function resetUserPassword(formData: FormData) {
  ...
  const { data: updated, error: profErr } = await supabase
    ...
    .single();
  if (profErr || !updated) {
    redirect(`/dashboard/users?error=${encodeURIComponent(friendlyError(profErr ?? 'User not found.'))}`);
  }
  ...
  if (password) {
    ...
    if (pwErr) {
      redirect(
        `/dashboard/users?error=${encodeURIComponent(`Username saved but password update failed: ${friendlyError(pwErr)}`)}`,
      );
    }
  }
