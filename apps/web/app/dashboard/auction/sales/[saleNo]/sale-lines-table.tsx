...
const COLUMNS: EntityListColumn<SaleLineRow>[] = [
  ...
  { key: "mfDate", label: "M/F date", accessor: (row) => row.mfDate ?? null, sortable: true, lov: false, searchInput: "date", render: (row) => row.mfDate ?? "—" },
  { key: "bagType", label: "F/H/B", accessor: (row) => row.bagType ?? null, sortable: true, filter: "select", render: (row) => row.bagType ?? "—" },
  { key: "chestType", label: "Type of chests", accessor: (row) => row.chestType ?? null, sortable: true, filter: "text", lov: false, render: (row) => row.chestType ?? "—" },
  { key: "chestNumbers", label: "Chest nos.", accessor: (row) => row.chestNumbers ?? null, sortable: true, filter: "text", lov: false, render: (row) => row.chestNumbers ?? "—" },
  {
    key: "moistureLevel",
    label: "Moisture %",
    accessor: (row) => row.moistureLevel ?? null,
    sortable: true,
    lov: false,
    searchInput: "number",
    headerClassName: "text-right",
