...
export function BundledDispatchForm({
  ...
  return (
    <EntityList
      ...
      initialRows={dateRows}
      definition={INVOICE_LIST}
      getId={(row) => row.id}
      rowLabel={(row) => `Broker Invoice ${row.invoiceNo}`}
      emptyMessage="No eligible Broker Invoices in this date range."
      renderMode="workflow"
      render={({ rows, visibleRows, controls, selection }) => {
        const eligibleIds = new Set(rows.map((invoice) => invoice.id));
                    ...
                    {visibleRows.length === 0 && (
                      <tr>
                        <td colSpan={INVOICE_LIST.columns.length + 1} className="px-4 py-8 text-center text-stone-500 dark:text-stone-400">
                          {rows.length === 0 ? "No eligible Broker Invoices in this date range." : "No eligible Broker Invoices match these filters."}
                        </td>
                      </tr>
                    )}
                  </tbody>
