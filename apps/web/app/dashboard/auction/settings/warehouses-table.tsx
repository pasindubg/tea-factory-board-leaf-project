...
export function WarehousesTable({ rows, isOwner }: { rows: WarehouseTableRow[]; isOwner: boolean }) {
  ...
  return <ListSurface>
    <ListCommandToolbar mode={LIST.selectionMode ?? "single"} count={selection.selectedCount} enableAdd={Boolean(isOwner && LIST.add)} onAdd={{ label: "Add warehouse", onClick: () => setAdding((value) => !value), disabled: Boolean(editingId) }} enableEdit={Boolean(isOwner && LIST.edit)} onEdit={{ label: "Edit", onClick: () => setEditingId(selection.selectedId), disabled: !selection.selectedId || Boolean(editingId) }}>
    ...
    <ListCreatePanel open={adding} title="Add warehouse"><form action={createAuctionWarehouse} className="flex max-w-xl gap-2"><input name="name" required placeholder="Main warehouse" className={input} /><SubmitButton pendingText="Adding…" className="shrink-0 rounded-full bg-green-700 px-4 text-sm font-semibold text-white">Add</SubmitButton></form></ListCreatePanel>
    ...
    <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-stone-200 text-left text-xs uppercase tracking-wide text-stone-500 dark:border-stone-700 dark:text-stone-400">{LIST.columns.map((column) => <th key={column.key} className="px-4 py-3">{column.sortable ? <SortButton col={column} controls={controls} /> : column.label}</th>)}</tr></thead><tbody>
      {visibleRows.map((warehouse) => {
        ...
        return <tr key={warehouse.id} {...selection.rowProps(warehouse.id, rowEditing)} className={`cursor-pointer border-b border-stone-100 last:border-0 dark:border-stone-800 ${selection.isSelected(warehouse.id) ? "bg-green-50/60 dark:bg-green-950/20" : ""}`}><td className="px-4 py-3">{rowEditing ? <><form id={formId} action={updateAuctionWarehouse.bind(null, warehouse.id)} /><input form={formId} name="name" required defaultValue={warehouse.name} className={input} /></> : warehouse.name}</td><td className="px-4 py-3">{rowEditing ? <label className="inline-flex items-center gap-2 text-xs text-stone-600 dark:text-stone-300"><input form={formId} name="active" type="checkbox" defaultChecked={warehouse.active} className="rounded border-stone-300" />Active</label> : <span className={`rounded-full px-2 py-0.5 text-xs ${warehouse.active ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300" : "bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-400"}`}>{warehouse.active ? "Active" : "Inactive"}</span>}</td></tr>;
      })}
